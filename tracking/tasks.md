...
- [TODO]   T-405 — "Recent failures by IP" tripwire panel (E17)
  *Top-of-page card on Login Activity: IPs with ≥10 failures in last 24h, count + last-seen timestamp + "view all" link that pre-fills the IP filter. Read-only; no auto-block.*
- [TODO]   T-406 — Retention setting + nightly cleanup job (E17)
  *New `system_settings` key `auth_audit.retention_days` (default 365). Nightly Nest scheduled task (`@Cron('0 3 * * *')`) deletes rows older than the configured window. Skipped when value is `0` (retain forever). Documented in `docs/04-deployment-guide.md`.*
